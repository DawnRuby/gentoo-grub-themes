...
Fonts can be generated by running the following command:
`grub-mkfont -s 16 -v /tmp/MYFONT.tff -o /boot/grub/fonts/MyFont`
The user should ensure to use the verbose flag to know how to reference the fonts in grub's design files.
Font size can be adjusted by the user via the -s flag.
